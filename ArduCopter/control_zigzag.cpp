...
void Copter::zigzag_calculate_next_dest(Vector3f& next_dest) const
{
    //get current position
    Vector3f cur_pos = inertial_nav.get_position();
...
void Copter::zigzag_receive_signal_from_auxsw(uint8_t aux_switch_position)
{
...
bool Copter::zigzag_set_destination(const Vector3f& destination, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw)
{
...
void Copter::zigzag_set_yaw_state(bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_angle)
{
    if (use_yaw) {
